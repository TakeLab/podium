# This code is copied from the hf/notebooks repo and tailored to our needs.
...
PATH_TO_DOCS = '..'
PATH_TO_DEST = '../notebooks'
DOC_BASE_URL = "http://takelab.fer.hr/podium/"
...
_re_section_underline = re.compile(r"=+|\^+|\-+|\"+|#+")
    ...
    for doc_file in Path('..').rglob("*.rst"):
        ...
        with open(doc_file, "r", encoding="utf-8") as f:
            ...
            for label in _re_label.finditer(doc_text):
                prev_line = None
                text_seen, underline_seen = False, False
                for line in doc_text[label.end() + 1:].splitlines():
                    line = line.strip()
                    if line == '' or line.startswith('..'):
                        if underline_seen:
                            ...
                            continue
                    if _re_section_underline.match(line):
                        if text_seen or underline_seen:
                            break
                        underline_seen = True
                    else:
                        if text_seen:
                        ...
                        text_seen = True
                section = prev_line
                ref2doc[label.group(1)] = (doc_file_relative_path, section)
    return ref2doc


REF2DOC = collect_refs()
...
def split_blocks(lines):
    ...
    def _move_to_next_non_empty_line(i):
        while i < len(lines) and (len(lines[i]) == 0 or (":options:" in lines[i] or ":skipif:" in lines[i])):
            i += 1
        return i
...
def convert_rst_links(text, rst_file):
    ...
    def _rep_ref(match, with_description=False):
        if with_description:
            title, ref = match.groups()
            ref_rst_file, _ = REF2DOC[ref]
        else:
            ref = match.group(1)
            ref_rst_file, title = REF2DOC[ref]
        if ref_rst_file == rst_file:
            link = '#' + ref
        else:
            link = urllib.parse.urljoin(
                DOC_BASE_URL, str(Path(ref_rst_file).with_suffix('.html'))
            )
            link = urllib.parse.urljoin(link, '#' + ref)
        return f"[{title if title is not None else ref}]({link})"
    # Refs of the form :ref:`page`
    # text = _re_simple_ref.sub(r'[\1](#\1)', text)
    text = _re_simple_ref.sub(_rep_ref, text)
    # Refs of the form :ref:`text <page>`
    # text = _re_ref_with_description.sub(r'[\1](#\2)', text)
    text = _re_ref_with_description.sub(partial(_rep_ref, with_description=True), text)
