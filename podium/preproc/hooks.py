...
from sacremoses import MosesPunctNormalizer, MosesTruecaser as MosesTruecaser_
...
LANGUAGES.update({lang: lang for lang in LANGUAGES.values()})
...
class MosesTruecaser:
    """Pretokenization hook that applies truecasing to the raw textual data.
    The truecaser can be trained and saved to file, from which it can be loaded
    for further use.

    Uses sacremoses.MosesTruecaser to perform truecasing.
    """

    def __init__(self,
                 filename: Optional[TextIO] = None,
                 save_to: Optional[TextIO] = None,
                 load_from: Optional[TextIO] = None,
                 use_known: bool = False
                 ) -> None:
        """MosesTruecaser constructor.

        Parameters
        ----------
        filename : file or str or Path, optional
            File from which the data is loaded to train the truecaser. Default: None.
        save_to : file or str or Path, optional
            File to which the trained truecaser is saved. Default: None.
        load_from : file or str or Path, optional
            File from which the trained truecaser is loaded. Default: None.
        use_known : bool, optional
            Use a known case if a word is a known word but not the first word.
            Default: None.

        Raises
        ------
        ValueError
            If filename is None and save_to is not None.
            If filename is specified and load_from is not None.
            If both save_to and load_from are specified.
        """

        if filename is None and save_to is not None:
            raise ValueError("If `filename` is not specified, `save_to` must be None.")

        if filename is not None and load_from is not None:
            raise ValueError("If `filename` is specified, `load_from` must be None.")

        if save_to is not None and load_from is not None:
            raise ValueError("Specifying both `save_to` and `load_from` is not allowed.")

        # if load_from is None, do nothing
        self._truecaser = MosesTruecaser_(load_from=load_from)

        if filename:
            self._truecaser.train_from_file(filename, save_to)

    def __call__(self, raw: str) -> str:
        """Applies truecasing to the raw textual data.
        ...
        -------
        str
            Truecased textual data.
        """
        return self._truecaser.truecase(raw, return_str=True, use_known=self.use_known)
...
class NLTKLemmatizer:
    """"Posttokenization hook that applies lemmatization to the tokenized textual data.

    Uses nltk.stem.WordNetLemmatizer to perform lemmatization.
    """

    def __init__(self) -> None:
        """NLTKLemmatizer constructor.
        """
        self._lemmatizer = WordNetLemmatizer()

    def __call__(self, raw: str, tokenized: List[str]) -> Tuple[str, List[str]]:
        """Lemmatizes the tokenized textual data. The raw part is left unchanged.
        ...
        """
        return raw, [self._lemmatizer.lemmatize(token) for token in tokenized]
...
class SpacyLemmatizer:
    ...
    def __init__(self, language: str = 'en') -> None:
        ...
        try:
            ...
            _LOGGER.warning(f"SpaCy model {language} not found."
...
class RegexReplace:
    ...
    def __init__(self,
                 replace_patterns: Sequence[Tuple[Union[re.Pattern, str], str]],
                 ) -> None:
        ...
        replace_patterns : sequence of tuple(Union[re.Pattern, str], str)
            ...
            the first element.
        """
        self._patterns = [(re.compile(pattern), repl)
