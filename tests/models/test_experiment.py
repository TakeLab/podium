...
def get_dataset():
    data = [
        {"Name": "Mark Dark", "Score": 5},
        {"Name": "Stephen Smith", "Score": 10},
        {"Name": "Ann Mann", "Score": 15},
    ]

    name_field = Field("Name", numericalizer=Vocab(), keep_raw=True, tokenizer="split")

    score_field = Field(
        "Score", numericalizer=int, keep_raw=True, tokenizer=None, is_target=True
    )
...
def test_experiment_train(fit_transformer):
    ...
    default_model_args = {"m_arg1": 1, "m_arg2": 2}

    default_trainer_args = {"t_arg1": 1, "t_arg2": 2}

    model_args = {"m_arg2": 3, "m_arg3": 4}

    trainer_args = {"t_arg2": 3, "t_arg3": 4}

    expected_model_args = {"m_arg1": 1, "m_arg2": 3, "m_arg3": 4}

    expected_trainer_args = {"t_arg1": 1, "t_arg2": 3, "t_arg3": 4}
    ...
    experiment.fit(test_dataset, model_args, trainer_args)
