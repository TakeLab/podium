...
from podium.datasets.impl.cornell_movie_dialogs import (
    CornellMovieDialogs,
...
TITLES_DATA = (
    "m0 +++$+++ 10 things i hate about you +++$+++ 1999 +++$+++ 6.90 +++$+++ "
...
URLS_DATA = (
    "m0 +++$+++ 10 things i hate about you +++$+++ http://www.dailyscript.com/sc"
    "ripts/10Things.html\n"
...
def create_mock_dataset(tmpdir):
    dataset_dir = os.path.join(
        tmpdir, CornellMovieDialogsLoader.NAME, CornellMovieDialogsLoader.DATA_FOLDER_NAME
    )
    os.makedirs(name=dataset_dir)
    create_file(
        file_path=os.path.join(
            dataset_dir, CornellMovieDialogsLoader.CHARACTERS_FILENAME
        ),
        file_content=CHARACTERS_DATA,
    )
    create_file(
        file_path=os.path.join(
            dataset_dir, CornellMovieDialogsLoader.CONVERSATIONS_FILENAME
        ),
        file_content=CONVERSATIONS_DATA,
    )
    create_file(
        file_path=os.path.join(dataset_dir, CornellMovieDialogsLoader.LINES_FILENAME),
        file_content=LINES_DATA,
    )
    create_file(
        file_path=os.path.join(dataset_dir, CornellMovieDialogsLoader.URL_FILENAME),
        file_content=URLS_DATA,
    )
    create_file(
        file_path=os.path.join(dataset_dir, CornellMovieDialogsLoader.TITLE_FILENAME),
...
def test_loading_dataset(tmpdir):
    ...
    with patch.object(LargeResource, "BASE_RESOURCE_DIR", tmpdir):
        ...
        assert len(conversations) == len(CornellMovieDialogsLoader.CONVERSATIONS_FIELDS)
...
def default_dataset():
    ...
    return CornellMovieDialogs(data=data)
