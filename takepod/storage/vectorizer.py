...
class BasicVectorStorage(VectorStorage):
    ...
    def load_vocab(self, vocab):
        if vocab is None:
            _LOGGER.error("Cannot load vectors for vocab because given "
                          "vocab is None.")
    ...
    def token_to_vector(self, token):
        if not self._initialized:
            _LOGGER.error("Vector storage is not initialized so it cannot"
            ...
            raise RuntimeError("VectorStorage is not initialized."
                               ...
                               " to initialize.")
        if token is None:
            _LOGGER.error("User gave None token to be converted to vector"
    ...
    def get_vector_dim(self):
        if not self._initialized:
            _LOGGER.error("Vector storage must be initialized to obtain "
                          "vector dimenstion.")
    ...
    def _check_path(self):
        ...
        if self._path is None and self._cache_path is None:
            _LOGGER.error("Error in checking paths that are handed to "
