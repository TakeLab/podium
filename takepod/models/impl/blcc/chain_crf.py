...
def add_boundary_energy(x, b_start=None, b_end=None, mask=None):
    ...
    if mask is None:
        ...
        x *= mask
        if b_start is not None:
            mask_r = K.concatenate(
                [K.zeros_like(mask[:, :1]), mask[:, :-1]],
                axis=1
            )
...
def viterbi_decode(x, U, b_start=None, b_end=None, mask=None):
    ...
    _, gamma = _forward(x,
                        lambda B: [
                            K.cast(K.argmax(B, axis=1), K.floatx()),
                            K.max(B, axis=1)
                        ],
                        initial_states,
                        U,
...
class ChainCRF(Layer):
    ...
    Given a ChainCRF instance crf both loss functions, crf.loss and
    crf.sparse_loss, return a tensor of shape (batch_size, 1) and not
    (batch_size, maxlen). that sample weighting in temporal mode.
    """
...
def create_custom_objects():
    ...
    return {
        'ChainCRF': ChainCRFClassWrapper,
        'ChainCRFClassWrapper': ChainCRFClassWrapper,
        'loss': loss,
